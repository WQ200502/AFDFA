...
private _cmds = [";god", {
		if (_alvl > 1 || getPlayerUID player isEqualTo "") then {
			if !(oev_godmode) then {
		...
		if (_alvl > 2 || (getPlayerUID player) isEqualTo "") then {
		...
		if (_alvl > 3 || _dlvl > 1) then {
			if ((getPlayerUID player) isEqualTo "") exitWith {};
		...
		if (getPlayerUID player in ["","","",""]) then { //Jay, Zahzi, Hylos, Ryan
			_index = [-1,-1];
			if(count _cmdArgs > 2) then {
				if((_cmdArgs select 2) find ".rvmat" != -1) then {
					_index = [parseNumber (_cmdArgs select 1), 2];
